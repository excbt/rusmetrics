 ...
 *
 */
public interface ReportSheduleRepository extends CrudRepository<ReportShedule, Long> {
	 ...
	 * @return
	 */
	@Query("SELECT rs FROM ReportShedule rs " + "LEFT JOIN FETCH rs.reportTemplate LEFT JOIN FETCH rs.reportParamset "
			+ "WHERE rs.subscriber.id = :subscriberId AND " + "rs.sheduleStartDate <= :sheduleDate AND "
			+ "( rs.sheduleEndDate IS NULL OR rs.sheduleEndDate >= :sheduleDate)")
	public List<ReportShedule> selectReportShedule(@Param("subscriberId") long subscriberId,
