...
/*global angular, moment, $, Blob, saveAs, alert*/
'use strict';
...
app.controller('ReportsCtrl', ['$scope', '$rootScope', '$http', 'crudGridDataFactoryWithCanceler', 'notificationFactory', 'objectSvc', 'mainSvc', '$timeout', 'reportSvc', '$q', '$filter', function ($scope, $rootScope, $http, crudGridDataFactoryWithCanceler, notificationFactory, objectSvc, mainSvc, $timeout, reportSvc, $q, $filter) {
    ...
    var CATEGORY_COEF = 1000,       /*"Весовой" коэффициент категории для определения последовательности вывода отчетов на экран*/
        ...
        serverTimeZone = mainSvc.getServerTimeZone(),/*in hours*/
...
//console.log(e);        
 ...
 //get templates   
    $scope.getActive = function () {
        ...
        var i;
        for (i = 0; i < $scope.reportObjects.length; i += 1) {
...
//console.log(mode);
            var flag;
            switch (mode) {
            case $scope.ctrlSettings.openModes.edit:
                activateMainPropertiesTab();
                break;
            case $scope.ctrlSettings.openModes.create:
                flag = $scope.checkRequiredFieldsOnSave();
                if (flag === false) {
...
//console.log(intervalValidate_flag);        
        result = !((($scope.currentObject.reportPeriodKey == null) || ($scope.currentObject.reportTemplate.id == null))) && intervalValidate_flag;
        ...
        $http(request)
            .then(function (response) {
                ...
                if ((file.type.indexOf("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet") > -1)) {
                    fileName += ".xlsx";
                } else if (file.type.indexOf('application/zip') > -1) {
                    fileName += ".zip";
                } else if (file.type.indexOf('application/pdf') > -1) {
                    fileName += ".pdf";
                } else if (file.type.indexOf('text/html') > -1) {
                    fileName += ".html";
                }
...
//console.log(tmpParamset);        
    ...
    $scope.setActiveTab = function (tabId) {
        var tab = null;
        tab = document.getElementById('rep_a_teplo_sys');
        tab.classList.remove("active");
        tab = document.getElementById('rep_a_electro_sys');
        tab.classList.remove("active");
        tab = document.getElementById('rep_a_gas_sys');
        tab.classList.remove("active");
...
//            rep.reportTemplate.reportTypeKeyname;
        ...
        if (reports.length > $scope.ctrlSettings.reportCountList) {
            ...
            reports.forEach(function (rep) {
                var categoryCounter;
                for (categoryCounter = 0; categoryCounter < $scope.data.reportEntities.length; categoryCounter += 1) {
...
//console.log(srcArray);        
    ...
    function removeCategoryRowsBeforeSave(spl) { /* spl - SpecialParamList*/
