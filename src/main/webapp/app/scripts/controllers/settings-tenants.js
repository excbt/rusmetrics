...
//var timeDirStart = (new Date()).getTime();
            ...
            $scope.messages.setMeterPeriods = "Назначить отчетные периоды";
...
//console.log($scope.objectsOnPage);                    
            ...
            window.onkeydown = function (e) {
                var elem = null;
                if (e.keyCode == 38) {
                    elem = document.getElementById("divWithObjectTable");
...
//****************************************************************************************
            ...
            var openMeterPeriodsModalFlag = false,
                multiPeriodSettingFlag = false; //false - period settings set for one object, true - for many objects
            ...
            function successGetMeterPeriodByObjectCallback(resp) {
                ...
                if (mainSvc.checkUndefinedNull(resp) || mainSvc.checkUndefinedNull(resp.data)) {
                    console.log("successGetMeterPeriodByObjectCallback: response is empty!");
                    return false;
                }
                setPeriodForObject(resp.data);
                //set meter period for current o
                $scope.data.resourceSystems.forEach(function (rs) {
                    if (!mainSvc.checkUndefinedNull(resp.data.meterPeriodSettings) && !mainSvc.checkUndefinedNull(resp.data.meterPeriodSettings[rs.keyname])) {
                        rs.value = resp.data.meterPeriodSettings[rs.keyname];
                    }
                });
                if (openMeterPeriodsModalFlag === true) {
            ...
            function loadObjectMeterPeriods() {
                objectSvc.loadObjectMeterPeriods().then(function (resp) {
//                    console.log(resp);
            ...
            $scope.setMeterPeriodsInit = function () {
                multiPeriodSettingFlag = true;
                $scope.data.resourceSystems = angular.copy(RESOURCE_SYSTEMS);
                $("#nmcMeterPeriodsModal").modal();
            };
            
            function prepareMeterPeriodSettings(inputData) {
                var result = {};
                ...
                return result.meterPeriodSettings;
            }
            
            function saveMeterPeriodForSelectedObjects(inputData) {
                ...
                var body = {
                    contObjectIds: objectIds,
                    meterPeriodSettings: prepareMeterPeriodSettings(inputData)
                };
                if (mainSvc.checkUndefinedNull(body.meterPeriodSettings)) {
                    console.log("Meter period settings is incorrect.");
                    return false;
                }
                objectSvc.setMeterPeriods(body).then(function (resp) {
                    console.log(resp);
                    if (mainSvc.checkUndefinedNull(resp) || mainSvc.checkUndefinedNull(resp.data) || !angular.isArray(resp.data)) {
                        console.log("Success callback for setMeterPeriods: response is empty!");
                        return false;
                    }
                    resp.data.forEach(function (omp) {
                        setPeriodForObject(omp);
...
//                subscrCabinetsSvc.createCabinets(objectIds).then(successCabinetsCallback, errorCallback);
            ...
            function saveMeterPeriodForOneObject(inputData) {
                //console.log("Input data:");
//console.log(inputData);
                var result = {};
                if (mainSvc.checkUndefinedNull($scope.data.currentObjectData) || mainSvc.checkUndefinedNull($scope.data.currentObjectData.contObjectInfo) || mainSvc.checkUndefinedNull($scope.data.currentObjectData.contObjectInfo.contObjectId)) {
                ...
                result.meterPeriodSettings = prepareMeterPeriodSettings(inputData);
//                inputData.forEach(function (systemMeterPeriodSetting) {
//                    if (!mainSvc.checkUndefinedNull(systemMeterPeriodSetting.value)) {
//                        if (mainSvc.checkUndefinedNull(result.meterPeriodSettings)) {
//                            result.meterPeriodSettings = {};
...
//console.log(result);
            ...
            $scope.saveMeterPeriodSettings = function (inputData) {
                if (multiPeriodSettingFlag === false) {
                    saveMeterPeriodForOneObject(inputData);
                } else if (multiPeriodSettingFlag === true) {
                    saveMeterPeriodForSelectedObjects(inputData);
                }
            };
