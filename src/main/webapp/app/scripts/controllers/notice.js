...
//console.log(el);
            ...
            if (!mainSvc.checkUndefinedNull(el.contEvent)) {
                ...
                case null:// oneNotice.noticeZpoint = "";
                    ...
                    break;
                default:
                    oneNotice.noticeZpoint = el.contServiceType;
...
//            oneNotice.zpointId = el.contEvent.contZPointId;
            ...
            oneNotice.noticeDate = $scope.dateFormat(el.contEventTimeDT);
...
//        get notice types.
        var i,
            j;
        for (i = 0; i < $scope.notices.length; i += 1) {
            if (noticeTypes.length > 0) {
                var flag = false;
                for (j = 0; j < noticeTypes.length; j += 1) {
...
//console.log($scope.notices[i]);
        ...
        for (i = 0; i < noticeTypes.length; i += 1) {
			data[i] = {
				label: noticeTypes[i],
