...
//console.log("ParamSetsCtrl");
    ...
    $scope.columns = [
        {"name": "reportType", "header": "Тип отчета", "class": "col-xs-11"}
    ];
    $scope.paramsetColumns = [
        {"name": "name", "header": "Наименование", "class": "col-xs-5"},
        /*,{"name": "reportTemplateName", "header": "Шаблон", "class": "col-xs-3"}*/
        {"name": "period", "header": "Период", "class": "col-xs-2"},
        {"name": "fileType", "header": "Тип файла", "class": "col-xs-1"},
        {"name": "outputFileNameTemplate", "header": "Шаблон имени файла", "class": "col-xs-3"}
    ];
...
//    29863766
    $scope.checkAndSaveParamset = function (paramsets, object) {
        ...
        if (!mainSvc.checkUndefinedNull(paramsets) && angular.isArray(paramsets)) {
            var psCounter;
            for (psCounter = 0; psCounter < paramsets.length; psCounter += 1) {
...
//console.log($scope.createParamset_flag);        
    ...
    $scope.toArchive = function (url, id) {
        return $resource(url, {}, { update: {method: 'PUT', params: {reportParamsetId: id}}});
    };
...
//        return result;
    ...
    $scope.setActiveTab = function (tabId) {
        var tab = null;
        tab = document.getElementById('a_teplo_sys');
        tab.classList.remove("active");
        tab = document.getElementById('a_electro_sys');
        tab.classList.remove("active");
        tab = document.getElementById('a_gas_sys');
        tab.classList.remove("active");
