...
app.controller('ObjectsMapCtrl', ['$rootScope', '$scope', '$cookies', 'objectSvc', 'mainSvc', '$timeout', function ($rootScope, $scope, $cookies, objectSvc, mainSvc, $timeout) {
...
//    objectSvc.sortObjectsByFullName($scope.objects);
    ...
    angular.extend($scope, {currentTile: currentTile});
...
//};
                ...
                case "heat":
                    heatSum += sys.absConsValue;
                    break;
                case "hw":
                    hwSum += sys.absConsValue;
                    break;
                }
            });
        });
    ...
    var getObjectsConsumingForObject = function (object, settings) {
        ...
        objectSvc.getObjectConsumingData(tmpSettings, object.contObject.id)
            .then(function (response) {
                ...
                var i,
                    curData = null;
                for (i = 0; i < $scope.newObjectData.serviceTypeARTs.length; i += 1) {
                    curData = $scope.newObjectData.serviceTypeARTs[i];
                ...
                curData = prepareObjectData(object);
